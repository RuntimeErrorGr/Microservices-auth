...
const PolicyEditor = () => {
  ...
  const handlePermissionChange = (resource, role, permission, checked) => {
    setPermissions(prev => ({
      ...prev,
      [resource]: [
        ...prev[resource].filter(p => !(p.role === role && p.permission === permission)),
  ...
  const savePermissions = async (resource) => {
    try {
      const response = await fetch(`/api/permissions/${resource}`, {
        method: 'POST',
        headers: {
      ...
      setSuccessMessage(`${resource} permissions updated successfully`);
